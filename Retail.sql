...
# So only the Markdowns. The dataset on kaggle has a description that says markdowns are not available pre Nov 2011 so this explaina some of this
# To deal with them we will impute the with 0, meianing no markdowns, taking inspiration from:
...
# May suggest that the markdown strategy starts with strong initial promotions (MarkDown1), slows mid-season (MarkDown3), and ends with high clearance discounts (MarkDown5 ).
...
# This could be because markdown 5 is used for non holiday promotions, or even post holiday promotions (e.g., clearance discounts as we mentioned earlier)

SELECT `Date`, AVG(MarkDown1) AS avg_md1
FROM feat_copy
GROUP BY `Date`
ORDER BY `Date`;
# Generally higher markdown1 over time (due to inflation for example)
...
# There is more EDA to do here but the rest will be done on other tools like Python using pandas. We can also start modelling using python and make predictions.
